...
// Root → Market Maker game
app.get('/', (req, res) => {
  res.sendFile('market-maker-game.html', { root: 'public' });
});
...
io.on('connection', (socket) => {
  ...
  socket.on('guest_action', (action) => {
    const roomCode = Object.keys(rooms).find(code => rooms[code].guests.some(g => g.socketId === socket.id));
    if (!roomCode) return;
    const room = rooms[roomCode];
  ...
  socket.on('chat', ({ message }) => {
    const roomCode = Object.keys(rooms).find(code => { const room = rooms[code]; return room.hostSocketId === socket.id || room.guests.some(g => g.socketId === socket.id); });
    if (!roomCode) return;
    const room = rooms[roomCode];
    let senderName = 'Unknown';
    if (room.hostSocketId === socket.id) senderName = room.hostName;
    else { const guest = room.guests.find(g => g.socketId === socket.id); if (guest) senderName = guest.name; }
  ...
  socket.on('disconnect', () => {
    ...
    const guestRoomCode = Object.keys(rooms).find(code => rooms[code].guests.some(g => g.socketId === socket.id));
    if (guestRoomCode) {
